...
### Task 3 — Asynchronous Auditing (Non-blocking HTTP)

- Fixed the blocking behaviour that was holding up HTTP request processing during audit DB writes.
- Introduced an **in-memory background worker** using `Channel<T>` as a message queue.
- When a Claim or Cover is **created or deleted**, a message is dispatched to the background worker — the HTTP response is returned immediately.
- The background worker then **asynchronously processes the message** and persists the audit entry to the **SQL Server** database.
